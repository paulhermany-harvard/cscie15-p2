...
<html lang="en">
  ...
  <body>
    ...
    <!-- Use a bootstrap navigation header for the top of the page. -->
    ...
    <div class="container">
        ...
        <!-- Use a bootstrap jumbotron to display the password in large type at the top of the page. -->
        <div class="jumbotron">
            <h1 id="password"><?=$password;?></h1>
        </div>
        
        <!-- The main form will post back to the index.php page using GET so that the settings can be saved in the url. -->
        <form id="chbs" name="chbs" action="index.php" method="GET">
            
            <div class="form-group">
                <!-- The main submit button is duplicated at the bottom of the form. -->
                <button type="submit" class="btn btn-primary btn-lg">Regenerate</button>
            </div>
            
            <!--
                The configuration of the form fields is set in the index.config.php file
                which includes configuration values for:
                    - the parameter name
                    - the display text
                    - deafult value
                    - constraints (min, max, values)
                    
                The default value of each field is set in the index.logic.php file so
                the value can be preserved on subsequent page requests.
            -->
            
            <!-- GRADE LEVEL -->
            <!--
                For the grade level field, the values are presented as a multi-select list
                and each option is given a data-rel-min and data-rel-max attribute to allow
                validation of the minLength and maxLength fields based on the selected
                grade level.
                
                For example, G1 words have a maximum of 9 characters, whereas SAT words
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="<?=$minLength_ParamName;?>" name="<?=$minLength_ParamName;?>" type="number"
                    value="<?=$minLength;?>"
                    min="<?=$minLength_Constraints['min'];?>"
                    max="<?=$minLength_Constraints['max'];?>"
                />
            </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="<?=$maxLength_ParamName;?>" name="<?=$maxLength_ParamName;?>" type="number"
                    value="<?=$maxLength;?>"
                    min="<?=$maxLength_Constraints['min'];?>"
                    max="<?=$maxLength_Constraints['max'];?>"
                />
            </div>
            ...
            <!-- ADD COMPLEXITY -->
            <!--
                For the complexity field, instead of presenting each option as a separate input control,
                they are presented in a human-readable sentence format. In fact, the request variables
                are named so that they can be read (sort of) by a human.
                
                For example, here is part of the querystring:
                    add=on&a_random=symbol&to_the=beginning&of=the+password
                    
                    ...it (literally) "adds on a random symbol to the beginning of the password"
                
                To do this cleanly, some Array/loop trickery is used.
            -->
            <div class="form-group">
                <label><?=$complexity_Text;?></label>
                <div class="checkbox">
                    <label>
            ...
            <div class="form-group">
                <!-- The second submit button is accompanied by a reset button, which simply redirects to the index.php page. -->
                <button type="submit" class="btn btn-primary btn-lg">Regenerate</button>
    ...
    <!-- Delegate the loading of javascript libraries to the "require" library -->
